...
module.exports = async function processImage (files, drawingEl, newRelIdCounterMap) {
  ...
  if (imageConfig.src && imageConfig.src.startsWith('data:')) {
    ...
    imageExtensions = extensionsParts.length === 1 ? extensionsParts[0] : extensionsParts[1]
    imageBuffer = Buffer.from(response.data)
    // console.log(imageBuffer)
    // here, if the buffer is invalid for whatever reason, it should be replaced with a placeholder
    try {
      sizeOf(imageBuffer)
    } catch (err) {
      // imageBuffer = Buffer.from('data:image/png;base64,'.split(';')[1].substring('base64,'.length), 'base64')
      imageConfig.src = 'https://via.placeholder.com/400x400.png?text=Bilddatei+defekt'
      const response = await axios({
        url: imageConfig.src,
        responseType: 'arraybuffer',
        method: 'GET'
