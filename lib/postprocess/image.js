...
// load placeholder in case an image fails to load or is broken
// const placeholderSrc2 = fs.readFileSync(path.join(__dirname, 'placeholder.png'), { encoding: 'base64' })
const placeholderSrc = fs.readFileSync(path.join(__dirname, 'placeholder.png'))
const placeholder = Buffer.from(placeholderSrc, 'base64')
...
module.exports = async files => {
  ...
  documentFile.data = await stringReplaceAsync(
    ...
    async val => {
      ...
      try {
        // sizeOf also checks for the right type and validity of the buffer
        imageDimension = sizeOf(imageBuffer)
      } catch (e) {
        imageExtensions = 'png'
